...
public class FileUploadServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String saveDirectory = "C:\\00.KOSMO93\\30.Web\\el_web_neon_work\\.metadata\\.plugins\\org.eclipse.wst.server.core\\tmp0\\wtpwebapps\\testJso\\fileupload";
		...
		try{
			// MultipartRequest (HTTPServletRequest, String, int, String, FileRenamePolicy)
			...
			// MultipartRequest 클래스의 참조변수를 이용해야 한다.
			...
			if (aList != null && aList.size() > 0){
				...
				// RequestDispatcher 인터페이스에서 사용하는 getReqeustDispatcher 함수에서는
				// Context 경로를 사용하지 않음
				RequestDispatcher rd = request.getRequestDispatcher("/cgiTest/fileUpload.jsp");
				rd.forward(request, response);
			} else {
				out.println("<script>");
				out.println("location.href='/testJso/cgiTest/fileUpload.html'");
				out.println("</script>");
			}
		}catch(Exception e){
